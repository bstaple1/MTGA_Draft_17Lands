...
class WindowUI:
    def __init__(self, root, filename, step_through, configuration):
        ...
        self.current_draft_label = Label(current_draft_label_frame, text="Current Draft:", font='Helvetica 9 bold',
                                         anchor="e")
        ...
        self.data_source_label = Label(data_source_label_frame, text="Data Source:", font='Helvetica 9 bold',
                                       anchor="e")

        deck_colors_label_frame = Frame(self.root)
        self.deck_colors_label = Label(deck_colors_label_frame, text="Deck Filter:", font='Helvetica 9 bold',
                                       anchor="e")
        ...
        self.data_source_options = OptionMenu(data_source_option_frame, self.data_source_selection,
                                              self.data_source_selection.get(), *self.data_source_list,
        ...
        self.deck_colors_options = OptionMenu(deck_colors_option_frame, self.column_4_selection,
                                              self.column_4_selection.get(), *self.column_4_list,
                                              style="my.TMenubutton")

        self.refresh_button_frame = Frame(self.root)
        self.refresh_button = Button(self.refresh_button_frame, command=lambda: self.UpdateCallback(True),
                                     text="Refresh");
        ...
        self.stat_options = OptionMenu(self.stat_frame, self.stat_options_selection, self.stat_options_list[0],
                                       *self.stat_options_list, style="my.TMenubutton")
        self.stat_options.config(width=11)

        citation_label = Label(self.root, text="Powered by 17Lands*", font='Helvetica 9 ', anchor="e", borderwidth=2,
                               relief="groove")
        hotkey_label = Label(self.root, text="CTRL+G to Minimize", font='Helvetica 8 ', anchor="e")
        footnote_label = Label(self.root, text="*This application is not endorsed by 17Lands", font='Helvetica 8 ',
                               anchor="e")
    ...
    def CreateHeader(self, frame, height, headers, total_width):
        ...
        try:
            ...
            list_box.tag_configure("whitecard", font=("Cascadia", 7, "bold"), background="#FFFFFF",
                                   foreground="#000000")
            ...
            list_box.tag_configure("blackcard", font=("Cascadia", 7, "bold"), background="#BFBFBF",
                                   foreground="#000000")
            list_box.tag_configure("greencard", font=("Cascadia", 7, "bold"), background="#60DC68",
                                   foreground="#000000")
            ...
            for count, column in enumerate(header_labels):
                list_box.column(column, stretch=NO, anchor=headers[column]["anchor"],
                                width=int(headers[column]["width"] * total_width))
    ...
    def UpdatePackTable(self, card_list, taken_cards, filtered):
        try:
            ...
            for count, card in enumerate(filtered_list):
                ...
                self.pack_table.insert("", index=count, iid=count, values=(
                card["name"], card["rating_filter_a"], card["rating_filter_b"], card["rating_filter_c"]),
                                       tag=(row_tag,))
            self.pack_table.bind("<<TreeviewSelect>>",
                                 lambda event: self.OnClickTable(event, table=self.pack_table, card_list=card_list,
                                                                 selected_color=filtered["filtered_c"]))
        except Exception as error:
    ...
    def UpdateMissingTable(self, current_pack, previous_pack, picked_cards, taken_cards, filtered):
        try:
            ...
            # Update the filtered column header with the filtered colors
            TableFilterOptions(self.missing_table, filtered["filtered_a"], filtered["filtered_b"],
                               filtered["filtered_c"])
            if len(previous_pack) == 0:
                ...
                if list_length:
                    ...
                    for count, card in enumerate(filtered_list):
                        ...
                        self.missing_table.insert("", index=count, iid=count, values=(
                        card_name, card["rating_filter_a"], card["rating_filter_b"], card["rating_filter_c"]),
                                                  tag=(row_tag,))
                    self.missing_table.bind("<<TreeviewSelect>>",
                                            lambda event: self.OnClickTable(event, table=self.missing_table,
    ...
    def UpdateCompareTable(self, compare_table, matching_cards, entry_box, card_list, filtered):
        try:
            ...
            if len(added_card):
                cards = [card_list[x] for x in card_list if
                         card_list[x]["name"] == added_card and card_list[x] not in matching_cards]
            ...
            for count, card in enumerate(filtered_list):
                row_tag = CL.RowColorTag(card["colors"])
                compare_table.insert("", index=count, iid=count, values=(
                card["name"], card["rating_filter_a"], card["rating_filter_b"], card["rating_filter_c"]),
                                     tag=(row_tag,))
            compare_table.bind("<<TreeviewSelect>>",
                               lambda event: self.OnClickTable(event, table=compare_table, card_list=matching_cards,
                                                               selected_color=filtered["filtered_c"]))
        except Exception as error:
    ...
    def UpdateTakenTable(self, taken_table, taken_cards, filtered):
        try:
            ...
            for count, card in enumerate(filtered_list):
                row_tag = CL.RowColorTag(card["colors"])
                taken_table.insert("", index=count, iid=count, values=(
                card["name"], card["rating_filter_a"], card["rating_filter_b"], card["rating_filter_c"]),
                                   tag=(row_tag,))
            taken_table.bind("<<TreeviewSelect>>",
                             lambda event: self.OnClickTable(event, table=taken_table, card_list=taken_cards,
                                                             selected_color=filtered["filtered_c"]))
        except Exception as error:
    ...
    def UpdateSuggestDeckTable(self, suggest_table, selected_color, suggested_decks, color_options):
        try:
            ...
            suggest_table.bind("<<TreeviewSelect>>",
                               lambda event: self.OnClickTable(event, table=suggest_table, card_list=suggested_deck,
    ...
    def UpdateDeckStatsTable(self, taken_cards, filter_type):
        try:
            taken_cards = [c for c in taken_cards if c not in self.draft.sideboard]
            filter = []
    ...
    def UpdateCallback(self, enable_draft_search):
        ...
        filtered = {}
        filtered["filtered_a"] = CL.ColorFilter(taken_cards, self.deck_colors[self.column_2_selection.get()],
                                                self.configuration)
        filtered["filtered_b"] = CL.ColorFilter(taken_cards, self.deck_colors[self.column_3_selection.get()],
                                                self.configuration)
        filtered["filtered_c"] = CL.ColorFilter(taken_cards, self.deck_colors[self.column_4_selection.get()],
                                                self.configuration)
    ...
    def CardComparePopup(self):
        ...
        try:
            ...
            clear_button = Button(popup, text="Clear",
                                  command=lambda: self.ClearCompareTable(compare_table, matching_cards))
        ...
        try:
            ...
            if len(suggested_decks):
                ...
                for color in suggested_decks:
                    rating_label = "%s %s (Rating:%d)" % (
                    color, suggested_decks[color]["type"], suggested_decks[color]["rating"])
    ...
    def AddSet(self, set, draft, start, end, button, progress, list_box, sets, version):
        ...
        while (True):
            try:
                message_box = MessageBox.askyesno(title="Download",
                                                  message=f"17Lands updates their card data once a day at 01:30 UTC. Are you sure that you want to download {set.get()} {draft.get()} data?")
                if not message_box:
                    break
                ...
                result, result_string, temp_size = self.extractor.RetrieveLocalArenaData(
                    self.configuration.database_size)
                if result == False:
                        ...
                        for count, color in enumerate(color_list):
                            try:
                                ...
                                if "gih" in card["deck_colors"][color]:
                                    ...
                                    gihwr = CL.CalculateWinRate(card["deck_colors"][color],
