        ...
        try:
            ...
            if len(name_segments) == 3:
                ...
                if ((name_segments[0].upper() in main_sets) and
                    ...
                    result, json_data = FileIntegrityCheck(
...
class FileExtractor:
    ...
    def DownloadCardData(self, ui_root, progress_bar, status, database_size):
        ...
        temp_size = 0
        try:
            result, result_string, temp_size = self.DownloadExpansion(
    ...
    def DownloadExpansion(self, ui_root, progress_bar, status, database_size):
        ...
        try:
            while (True):
                ...
                result, result_string, temp_size = self.RetrieveLocalArenaData(
    ...
    def RetrieveLocalArenaData(self, root, status, previous_database_size):
        ...
        arena_cards_locations = SearchLocalFiles(
        ...
        while (True):
            try:
                ...
                if current_database_size != previous_database_size:
                    file_logger.info(
                        f"Local File Change Detected {current_database_size}, {previous_database_size}")
                    file_logger.info(
                        f"Local Database Data: Searching file path {arena_database_locations[0]}")
                    status.set("Retrieving Localization Data")
                    root.update()
                    result, card_text, card_enumerators = self.RetrieveLocalDatabase(
                    ...
                    file_logger.info(
                        f"Local Card Data: Searching file path {arena_cards_locations[0]}")
                    status.set("Retrieving Raw Card Data")
                    root.update()
                    result, raw_card_data = self.RetrieveLocalCards(
                        arena_cards_locations[0])
                    ...
                    root.update()
                    result = self.AssembleStoredData(
                ...
                root.update()
                result = self.RetrieveStoredData(
    ...
    def RetrieveLocalDatabase(self, file_location):
        ...
        try:
            ...
            while (True):
                ...
                result, card_enumerators = self.RetrieveLocalCardEnumerators(
                    rows)
        ...
        try:
            ...
            if constants.SET_SELECTION_ALL in set_list:
                ...
                for search_set in set_list:
                    matching_sets = list(
                        filter(lambda x: search_set in x, json_data.keys()))
    ...
    def Initialize17LandsData(self):
        for card in self.card_dict:
            ...
            for color in self.deck_colors:
                self.card_dict[card][constants.DATA_FIELD_DECK_COLORS][color] = {
                    x: 0.0 for x in constants.DATA_FIELD_17LANDS_DICT.keys() if x != constants.DATA_SECTION_IMAGES}
                ...
                for k, v in constants.DATA_FIELD_17LANDS_DICT.items():
                    if k == constants.DATA_SECTION_IMAGES:
                        ...
                        if (k in constants.WIN_RATE_OPTIONS) or (k == constants.DATA_FIELD_IWD):
                            color_data[colors][k] = round(
                                float(card[v]) * 100.0, 2) if card[v] != None else 0.0
        ...
        try:
            ...
            for color in colors:
                ...
                if (color["is_summary"] == False) and (games > 5000):
                    ...
                    color_label = [
                        x for x in color_ratings_dict.keys() if x in color_name]
    ...
    def ProcessCardRatings(self, card):
        ...
        try:
            ...
            matching_cards = [
                x for x in self.card_ratings.keys() if x in card_sides]
            if (matching_cards):
    ...
    def ProcessSetData(self, sets, data):
                    ...
                    if set["set_type"] == constants.SET_TYPE_ALCHEMY:
                        ...
                        if ("parent_set_code" in set) and ("block_code" in set):
                            sets[set_name][constants.SET_LIST_ARENA] = [
                                set["parent_set_code"].upper()]
                            sets[set_name][constants.SET_LIST_17LANDS] = [
                                f"{set['block_code'].upper()}{set['parent_set_code'].upper()}"]
                            sets[set_name][constants.SET_LIST_SCRYFALL] = [
                                set_code.upper(), set["parent_set_code"].upper()]
                        else:
        ...
        sets["Arena Cube"][constants.SET_START_DATE] = DateShift(
